...
const oldInventors = inventors.filter(
  (num) => num.year >= 1500 && num.year <= 1599
);
...
const firstLast = inventors.map(({ first, last }) => ({ [first]: last }));
...
const yearsAlive = inventors.sort((a, b) => {
  // return (a.passed - a.passed) - (b.year - b.year )
...
const yearsLive = inventors.reduce((a, b) => {
...
const reversedPeople = people.map((string) => string).reverse();

console.log(reversedPeople.join(" "));
...
const obj = data.reduce((number, index, array) => {
...
const somelegalAge = devs.some((devs) => {
  const year = 2021;
  const age = year - devs.year;
  return age >= 19;
});

console.log(somelegalAge);

// // Array.prototype.every()
// // 9. Check if everyone is 19 or older?

const allLegalAge = devs.every((devs) => {
  const year = 2021;
  const age = year - devs.year;
  return age >= 19;
});
...
console.log(comments.indexOf("Ramen is my fav food ever"));
